...
def get_pose_estimation_in_euler_angle(landmark_shape, im_szie):
    try:
        ...
        return -1, None, None, None
def extract_euler_angle(filename):
...
def main():
    ...
    predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")
    POINTS_NUM_LANDMARK = 68
    for label1 in os.listdir('/media/ustb/Dataset/biovid/PartA/front_img2'):
        root_dir = '/media/ustb/Dataset/biovid/PartA/front_img2/' + label1
        filename2 = []
        label_descr = []
        for subdir1 in os.listdir(root_dir):
            if subdir1 != '.DS_Store':
                filename2.append(subdir1)
                ...
                for filename1 in files:
                    ...
                    angle_filename = '/media/ustb/Dataset/biovid/PartA/frame/' + label1 + '/' + subdir1 + '/' + filename1
                    print(angle_filename)
                    ...
                    deter,pitch,yaw,roll = extract_euler_angle(angle_filename)
                    if deter == 0:
                        feature.append(feature1)
                        continue
                    feature1 = [debl, debr, del1, der1, dbml, dbmr, deml, demr, dmw1, dwh1,gra1,gra2,gra3,pitch,yaw,roll]
                    feature.append(feature1)
                np_feature = np.array(feature)
                seq_descr = extract_signal(np_feature).tolist()
                label_descr.append(seq_descr[0])

        filename_1.append(filename2)
        all_descr.append(label_descr)
    all_descr = np.array(all_descr)
    filename_1 = np.array(filename_1)
    np.save('new_result1/parta_dis_texture_eulur_descriptor.npy',all_descr)
    np.save('new_result1/parta_name.npy',filename_1)
